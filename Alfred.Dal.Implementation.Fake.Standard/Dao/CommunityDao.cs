...
namespace Alfred.Dal.Implementation.Fake.Standard.Dao
{
    public class CommunityDao : ICommunityDao
    {
        ...
        {
            return _entityFactory.TransformToCommunityEntity(await Task.Run(() => FakeCommunitiesDb.Communities.FirstOrDefault(x => string.Equals(x.Email, email, StringComparison.CurrentCultureIgnoreCase))).ConfigureAwait(false));
        }
